...
joint_constants <- list(N = N, n = n, time = t, LOD = LOD,
                        c = ifelse(surv_data$status==1, Inf, surv_data$observe_time)) ## c: right-censoring time
...
## fit a linear mixed effects model on marker1
...
## fit a tobit mixed effects model on marker2
...
for(i in 1:3){
  Ucorr_b1_sample <- array(dim = c(n.sims, 2,2))
  Ucorr_b1_sample[,1,1] <- chain_output[[i]]$samples[,"Ucorr_b1[1, 1]"]
  Ucorr_b1_sample[,1,2] <- chain_output[[i]]$samples[,"Ucorr_b1[1, 2]"]
  Ucorr_b1_sample[,2,1] <- chain_output[[i]]$samples[,"Ucorr_b1[2, 1]"]
  Ucorr_b1_sample[,2,2] <- chain_output[[i]]$samples[,"Ucorr_b1[2, 2]"]
  corr_b1_sample <- sapply(1:n.sims, FUN = function(t) crossprod(Ucorr_b1_sample[t,,]))
  Omega_b1_sample[[i]] <- corr_b1_sample[2,]
}

Omega_b2_sample <- list()
for(i in 1:3){
  Ucorr_b2_sample <- array(dim = c(n.sims, 2,2))
  Ucorr_b2_sample[,1,1] <- chain_output[[i]]$samples[,"Ucorr_b2[1, 1]"]
  Ucorr_b2_sample[,1,2] <- chain_output[[i]]$samples[,"Ucorr_b2[1, 2]"]
  Ucorr_b2_sample[,2,1] <- chain_output[[i]]$samples[,"Ucorr_b2[2, 1]"]
  Ucorr_b2_sample[,2,2] <- chain_output[[i]]$samples[,"Ucorr_b2[2, 2]"]
  corr_b2_sample <- sapply(1:n.sims, FUN = function(t) crossprod(Ucorr_b2_sample[t,,]))
  Omega_b2_sample[[i]] <- corr_b2_sample[2,]
}

## make a traceplot of the posterior samples
pdf(paste0(k, ".result/traceplot.pdf"))
coda::traceplot(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta1",col)]),
                          mcmc(chain_output[[2]]$samples[,grep("^beta1",col)]),
                          mcmc(chain_output[[3]]$samples[,grep("^beta1",col)])))
...
coda::traceplot(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b1",col)]),
                          mcmc(chain_output[[2]]$samples[,grep("^sigma_b1",col)]),
                          mcmc(chain_output[[3]]$samples[,grep("^sigma_b1",col)])))
coda::traceplot(mcmc.list(mcmc(Omega_b1_sample[[1]]),
                          mcmc(Omega_b1_sample[[2]]),
                          mcmc(Omega_b1_sample[[3]])))
coda::traceplot(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta2",col)]),
                          mcmc(chain_output[[2]]$samples[,grep("^beta2",col)]),
                          mcmc(chain_output[[3]]$samples[,grep("^beta2",col)])))
...
coda::traceplot(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b2",col)]),
                          mcmc(chain_output[[2]]$samples[,grep("^sigma_b2",col)]),
                          mcmc(chain_output[[3]]$samples[,grep("^sigma_b2",col)])))
...
beta1_est <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta1",col)]),
                               mcmc(chain_output[[2]]$samples[,grep("^beta1",col)]),
                               mcmc(chain_output[[3]]$samples[,grep("^beta1",col)])))$statistics[,1]
...
sigma_b1_est <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b1",col)]),
                                  mcmc(chain_output[[2]]$samples[,grep("^sigma_b1",col)]),
                                  mcmc(chain_output[[3]]$samples[,grep("^sigma_b1",col)])))$statistics[,1]
Omega_b1_est <- summary(mcmc.list(mcmc(Omega_b1_sample[[1]]),
                                  mcmc(Omega_b1_sample[[2]]),
                                  mcmc(Omega_b1_sample[[3]])))$statistics[1]
beta2_est <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta2",col)]),
                               mcmc(chain_output[[2]]$samples[,grep("^beta2",col)]),
                               mcmc(chain_output[[3]]$samples[,grep("^beta2",col)])))$statistics[,1]
...
sigma_b2_est <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b2",col)]),
                                  mcmc(chain_output[[2]]$samples[,grep("^sigma_b2",col)]),
                                  mcmc(chain_output[[3]]$samples[,grep("^sigma_b2",col)])))$statistics[,1]
...
beta1_CI <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta1",col)]),
                              mcmc(chain_output[[2]]$samples[,grep("^beta1",col)]),
                              mcmc(chain_output[[3]]$samples[,grep("^beta1",col)])))$quantiles[,c(1,5)]
...
sigma_b1_CI <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b1",col)]),
                                 mcmc(chain_output[[2]]$samples[,grep("^sigma_b1",col)]),
                                 mcmc(chain_output[[3]]$samples[,grep("^sigma_b1",col)])))$quantiles[,c(1,5)]
Omega_b1_CI <- summary(mcmc.list(mcmc(Omega_b1_sample[[1]]),
                                 mcmc(Omega_b1_sample[[2]]),
                                 mcmc(Omega_b1_sample[[3]])))$quantiles[c(1,5)]
beta2_CI <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta2",col)]),
                              mcmc(chain_output[[2]]$samples[,grep("^beta2",col)]),
                              mcmc(chain_output[[3]]$samples[,grep("^beta2",col)])))$quantiles[,c(1,5)]
...
sigma_b2_CI <- summary(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b2",col)]),
                                 mcmc(chain_output[[2]]$samples[,grep("^sigma_b2",col)]),
                                 mcmc(chain_output[[3]]$samples[,grep("^sigma_b2",col)])))$quantiles[,c(1,5)]
...
beta1_Rhat <- coda::gelman.diag(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta1",col)]),
                                          mcmc(chain_output[[2]]$samples[,grep("^beta1",col)]),
                                          mcmc(chain_output[[3]]$samples[,grep("^beta1",col)])))$psrf[,1]
...
sigma_b1_Rhat <- coda::gelman.diag(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b1",col)]),
                                             mcmc(chain_output[[2]]$samples[,grep("^sigma_b1",col)]),
                                             mcmc(chain_output[[3]]$samples[,grep("^sigma_b1",col)])))$psrf[,1]
Omega_b1_Rhat <- coda::gelman.diag(mcmc.list(mcmc(Omega_b1_sample[[1]]),
                                             mcmc(Omega_b1_sample[[2]]),
                                             mcmc(Omega_b1_sample[[3]])))$psrf[,1]

beta2_Rhat <- coda::gelman.diag(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^beta2",col)]),
                                          mcmc(chain_output[[2]]$samples[,grep("^beta2",col)]),
                                          mcmc(chain_output[[3]]$samples[,grep("^beta2",col)])))$psrf[,1]
...
sigma_b2_Rhat <- coda::gelman.diag(mcmc.list(mcmc(chain_output[[1]]$samples[,grep("^sigma_b2",col)]),
                                             mcmc(chain_output[[2]]$samples[,grep("^sigma_b2",col)]),
                                             mcmc(chain_output[[3]]$samples[,grep("^sigma_b2",col)])))$psrf[,1]
